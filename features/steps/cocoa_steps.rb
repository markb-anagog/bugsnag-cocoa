...
def check_device_model(field, list)
  ...
  Maze.check.true(valid_models != nil ? valid_models.include?(device_model) : true, "The field #{device_model} did not match any of the list of expected fields")
end
...
Then('the breadcrumb timestamps are valid for the event') do
  ...
  unless device['time'].nil?
    ...
    breadcrumbs.each do |breadcrumb|
      Maze.check.true(breadcrumb['timestamp'] <= device['time'],
...
Then('the thread information is valid for the event') do
  ...
  Maze.check.equal(1, err_thread_count, "Expected errorReportingThread to be reported once for threads #{thread_traces}")
  ...
  stack_traces.each_with_index do |frame, index|
    ...
    Maze.check.equal(frame, thread_frame, "Thread and stacktrace differ at #{index}. Stack=#{frame}, thread=#{thread_frame}")
  end
end
